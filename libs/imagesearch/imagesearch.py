...
def imagesearcharea(image, x1,y1,x2,y2, precision=0.8, im=None) :
    ...
    #absolute values plz
    auxMax_loc = (max_loc[0] + x1,max_loc[1] + y1)

    return auxMax_loc #max_loc
...
def click_image(image,pos,  action, timestamp,offset=5):
    ...
    '''
    auxx = r(pos[0] + offset ,width - offset)
    auxy = r(pos[1] + offset ,height - offset)
    '''
    xoffset = (width * (offset / 100)) / 2
    yoffset = (height * (offset / 100)) / 2
    auxx = r(pos[0] + xoffset, width - xoffset * 2)
    auxy = r(pos[1] + yoffset, height - yoffset * 2)
    pyautogui.moveTo(auxx, auxy, timestamp)
    pyautogui.click(button=action)
    #print(time.strftime("%H:%M:%S") + " Clicked image at: [" + str(int(auxx)) + ", " + str(int(auxy)) + "]")
